...
class management(Communication):
    ...
    def __init__(self, market_event_securities , market_event_queue, securities, queue, host,
        ...
        self.alpha = self._init_market_dict(self.market_event_securities, self.market_event_queue)
    ...
    def _update_cash(self, size, price):
        ...
        print("cash balance: %d" % self.cash_balance)

    def _update_pnl(self,):
        self.PnL = self.cash_balance + self.inventoryValue
        print(" [X] portfolio value: %d" % self.PnL)
    ...
    def _get_signal(self,symbol, lv):
        # df = pd.DataFrame(self.observations,columns=self.securities)
        # df = df.reset_index()
        # df['ma'] = pd.rolling_mean(df[sym], 50)
        # df['26 ema'] = pd.ewm(df[sym], span=26).mean()
        # df['12 ema'] = pd.ewm(df[sym], span=12).mean()
        # df['MACD'] = (df['12 ema'] - df['26 ema'])
        # df['Signal'] = pd.ewm(df['MACD'], span=9).mean()
        # if df['MACD'] - df['Signal']
        # return 

        self.alpha[symbol][lv] = 1
    ...
    def _model_reaction_to_level(self, tob):
        ...
        # print(observation)
        # actions, _states = self.model.predict(observation)
    def _generate_orders(self,actions):
        print("action")
        print(actions)
        print("inventory")
        ...
        if self._condition_to_make_prediction():
            ...
            # new_inventory *= 10000
            new_inventory /= (np.sum(np.abs(new_inventory)) + 1e-9)
...
if __name__ == "__main__":
    ...
    window_size = 100  # number of orders for each security that the bot should receive to make an action
